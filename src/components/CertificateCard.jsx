...
const CertificateCard = ({ cert, onClick }) => {
  ...
  const handleDownload = (e) => {
    e.stopPropagation();
    // FIX 2: Updated "certificate." to "cert."
  ...
  return (
    ...
    >
      <div className="aspect-4/3 w-full overflow-hidden relative z-10 ">
        <img 
          src={cert.image} // FIX 3: Updated to cert.image
          alt={cert.title} // FIX 4: Updated to cert.title
        ...
        <div className='absolute bottom-0 left-0 right-0 p-6 bg-linear-to-t from-gray-900 via-gray-900/80 to-transparent translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300 ease-out z-20'>
            <h3 className="text-xl font-bold text-white mb-1 line-clamp-1">{cert.title}</h3> {/* FIX 5 */}
        <p className="text-gray-300 text-sm font-medium mb-4">Issuer: <span className="text-blue-200">{cert.issuer}</span></p> {/* FIX 6 */}
